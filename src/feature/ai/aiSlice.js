    ...
    async (taskText, { rejectWithValue }) => {
        try {
            const response = await fetch(BASE_URL, {
                ...
                body: JSON.stringify({
                    contents: [{ parts: [{ text: `Give useful insights or recommendations for this task in one line or in few words.give sentence like you are speeking to person: "${taskText}"` }] }]
                })
            });
