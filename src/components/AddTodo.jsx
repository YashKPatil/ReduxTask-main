...
function AddTodo({ editTodo, setEditTodo }) {
    ...
    return (
        <form onSubmit={addTodoHandler} className="flex flex-col gap-4 bg-white/90 p-5 rounded-lg shadow-md">
            <input
                type="text"
                value={input}
                ...
                <div className="flex justify-between">
                    {["High", "Medium", "Low"].map((level) => (
                        <label key={level} className={`cursor-pointer px-5 py-2 rounded-md border-2 font-medium transition-all duration-200 
                            ...
                            <input type="radio" name="priority" value={level} checked={priority === level} onChange={() => setPriority(level)} className="hidden"/>
                            {level}
                        </label>
                    ))}
                </div>
            ...
            <button type="submit" className="bg-blue-500 hover:bg-blue-600 text-white p-3 rounded-lg shadow-md transition-all duration-200">
